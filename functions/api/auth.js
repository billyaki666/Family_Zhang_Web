...
export async function onRequestPost({request,env}){
  try{
    ...
    if(body.action==="resetPassword"){
      ...
      const temporaryPassword=`Zs${crypto.randomUUID().replace(/-/g,"").slice(0,10)}`,secure=await hashPassword(temporaryPassword);user.passwordSalt=secure.salt;user.passwordDigest=secure.digest;delete user.passwordHash;user.passwordResetAt=new Date().toISOString();await writeState(env.DB,state);return json({temporaryPassword});
    }
    if(body.action==="login"){
      ...
      if(!user.passwordDigest){const secure=await hashPassword(String(body.password));user.passwordSalt=secure.salt;user.passwordDigest=secure.digest;delete user.passwordHash}
