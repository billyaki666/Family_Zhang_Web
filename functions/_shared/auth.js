...
export async function hashPassword(password,salt=randomHex(16)){const key=await crypto.subtle.importKey("raw",encoder.encode(password),"PBKDF2",false,["deriveBits"]);const bits=await crypto.subtle.deriveBits({name:"PBKDF2",hash:"SHA-256",salt:encoder.encode(salt),iterations:120000},key,256);return {salt,digest:bytesToHex(new Uint8Array(bits))}}
...
export async function verifyPassword(user,password){if(user.passwordDigest&&user.passwordSalt)return (await hashPassword(password,user.passwordSalt)).digest===user.passwordDigest;return user.passwordHash===legacyHash(password)}
...
export function publicUser(user){if(!user)return null;const {passwordHash,passwordDigest,passwordSalt,...safe}=user;return safe}
